...
#if defined(CONFIG_SENSORS_FPC_1020_SANDERS)
	mdata = kzalloc(sizeof(struct FPS_data), GFP_KERNEL);
#else
	mdata = devm_kzalloc(dev, sizeof(struct FPS_data), GFP_KERNEL);
#endif
	if (mdata) {
...
#if defined(CONFIG_SENSORS_FPC_1020_SANDERS)
#define FPC_MAX_UP_TIME 5000
#else
#define FPC_MAX_UP_TIME (1 * MSEC_PER_SEC)
#endif
...
static irqreturn_t fpc1020_irq_handler(int irq, void *handle)
{
	...
	pm_wakeup_event(fpc1020->dev, FPC_MAX_UP_TIME);
	dev_dbg(fpc1020->dev, "%s\n", __func__);
	fpc1020->irq_cnt++;
#if defined(CONFIG_SENSORS_FPC_1020_SANDERS)
	sysfs_notify(&fpc1020->dev->kobj, NULL, dev_attr_irq.attr.name);
#elif defined(CONFIG_INPUT_MISC_FPC1020_SAVE_TO_CLASS_DEVICE)
